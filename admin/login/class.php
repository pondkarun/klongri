...
function et($tag){
    ...
    $pt['ChkVer'] = 'Verificar por nova vers?o';
    ...
    $de1['ConfExec'] = 'Best?tige AUSFUHRUNG';
    ...
    $de1['ChkVer'] = 'Pr¨¹fe auf neue Version';
    ...
    $de2['ConfExec'] = 'Best?tigung AUSFUHREN';
    ...
    $de2['ChkVer'] = 'Uberpr¨¹fe neuer Version';
    ...
    $fr2['FileNoOverw'] = 'Fichier ne peut pas ¨ºtre ¨¦cras¨¦';
    ...
    $fr3['FileNoOverw'] = 'Fichier ne pouvant ¨ºtre remplac¨¦';
    $fr3['FileOverw'] = 'Fichier remplac¨¦';
    $fr3['FileIgnored'] = 'Fichier ignor¨¦';
    $fr3['ChkVer'] = 'V¨¦rifier nouvelle version';
...
function remove_special_chars($str){
    $str = trim($str);
    $str = strtr($str,"£¤¦ÌA?A????CEEEE??II????O???U?UU??¨¤?a?????¨¨¨¦¨º?????????????¨´??¨¹??!@#%&*()[]{}+=?",
        ...
        if ($handle = @opendir($dir_current)){
            ...
            if (count($mat_dir)){
                ...
                if ($dir_before != $dir_current){
                    if (strstr($expanded_dir_list,":$dir_current/$dir_name")) $op_str = "[¨C]";
                    else $op_str = "[+]";
...
function view(){
	...
	if (intval($passthru)){
	    ...
	    if ($is_reachable_thru_webserver){
	        ...
	        // Malditas variaveis de sistema!! No windows doc_root ¨¦ sempre em lowercase... cad¨º o str_ireplace() ??
...
class zip_file extends archive
{
    ...
    function create_zip()
    {
        ...
        foreach($this->files as $current)
        {
            ...
            $translate =  array('C'=>pack("C",128),'¨¹'=>pack("C",129),'¨¦'=>pack("C",130),'a'=>pack("C",131),'?'=>pack("C",132),
                                '¨¤'=>pack("C",133),'?'=>pack("C",134),'?'=>pack("C",135),'¨º'=>pack("C",136),'?'=>pack("C",137),
                                '¨¨'=>pack("C",138),'?'=>pack("C",139),'?'=>pack("C",140),'?'=>pack("C",141),'?'=>pack("C",142),
                                '?'=>pack("C",143),'E'=>pack("C",144),'?'=>pack("C",145),'?'=>pack("C",146),'?'=>pack("C",147),
                                '?'=>pack("C",148),'?'=>pack("C",149),'?'=>pack("C",150),'¨´'=>pack("C",151),'_'=>pack("C",152),
                                '?'=>pack("C",153),'U'=>pack("C",154),'¡ê'=>pack("C",156),'£¤'=>pack("C",157),'_'=>pack("C",158),
